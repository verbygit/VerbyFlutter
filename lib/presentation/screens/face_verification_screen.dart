...
class _FaceVerificationScreenState
    ...
    try {
      ...
      result.fold(
          ...
          if (faceModel != null) {
            ...
            print(
              '✅ Loaded registered face for employee: ${faceModel
                  .employeeName}',
            );
  ...
  void _initFaceDetector() {
    final options = FaceDetectorOptions(
      ...
      enableLandmarks: true, // Enable landmarks for better eye detection
      enableClassification: true, // Enable classification for eye detection
      enableTracking: false,
      minFaceSize: 0.05, // Even smaller minimum face size
    );
    _faceDetector = FaceDetector(options: options);
    print('🔧 Face detector initialized with accurate mode, minFaceSize: 0.05, landmarks and classification enabled');
  }
  ...
  Future<void> _initCamera() async {
    try {
      ...
      setState(() {
        _status = 'Camera initialized successfully. Loading registered face...';
      });
      } else {
        setState(() {
    ...
    try {
      ...
      if (faces.isEmpty) {
        ...
        if (width < minPixelSize || height < minPixelSize || (cameraResolution != null && faceSizeRatio < minFaceSizeRatio)) {
          ...
          print(
            '🎯 Face detected - Size: ${width.toInt()}x${height
                .toInt()}, Orientation: [${orientation[0].toStringAsFixed(
                1)}, ${orientation[1].toStringAsFixed(1)}, ${orientation[2]
                .toStringAsFixed(1)}]',
          );
          ...
          if (isCentered) {
            ...
            if (croppedFace != null) {
              ...
              if (recognition != null) {
                print(
                  '🔍 Face recognition result: ID=${recognition
                      .id}, Distance=${recognition.distance.toStringAsFixed(
                      3)}, Name=${recognition.name}',
                );
  ...
  void _analyzeFace(Face face, Recognition recognition) {
    ...
    if (leftEyeProb != null && rightEyeProb != null) {
      ...
      print('👁️ Eye detection: Left=${leftEyeProb.toStringAsFixed(2)} (closed=$leftClosed), Right=${rightEyeProb.toStringAsFixed(2)} (closed=$rightClosed), Closed=$eyesClosed');
    } else if (leftEyeProb != null) {
      // Only left eye probability available
      eyesClosed = leftEyeProb < 0.35;
      print('👁️ Eye detection: Left=${leftEyeProb.toStringAsFixed(2)}, Right=null, Closed=$eyesClosed');
    } else if (rightEyeProb != null) {
      // Only right eye probability available
      eyesClosed = rightEyeProb < 0.35;
      print('👁️ Eye detection: Left=null, Right=${rightEyeProb.toStringAsFixed(2)}, Closed=$eyesClosed');
    } else {
      // No eye probabilities available - assume eyes are open (don't trigger blink detection)
      eyesClosed = false;
      print('👁️ Eye detection: No probabilities available, assuming eyes open');
    }
    
    print('👁️ Current eye state: $_currentEyeState, Closed frames: $_eyesClosedFrames');
    ...
    const double strictDistanceThreshold = 1.2; // Adjusted to 1.2 for better usability while maintaining security
    
    if (recognition.id == widget.employee.id && recognition.distance < strictDistanceThreshold) {
      print('✅ Face verification PASSED: ID matches and distance is acceptable');
      print('🔍 Face recognized flag: $_isFaceRecognized, Distance: ${recognition.distance}');
      if (!_isFaceRecognized) {
      ...
      switch (_currentEyeState) {
        ...
        case EyeState.eyesClosed:
          if (eyesClosed) {
            _eyesClosedFrames++;
            print('👁️ Eye state: Closed (frames: $_eyesClosedFrames/$_closedThreshold)');
          ...
          if (!eyesClosed) {
            ...
            if (!isNavigatedForward) {
              ...
              navigatePushAndRemoveUntil(
                  context, SelectOperationScreen(employee: widget.employee),
                  true);
            }
          }
          break;
      ...
      _updateStatus('Face does not match registered employee');
        ...
        print('❌ Face verification FAILED: Distance too high (${recognition.distance.toStringAsFixed(3)} >= $strictDistanceThreshold)');
